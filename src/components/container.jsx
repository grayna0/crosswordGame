...
// export default function KeywordCrossword({ data }) {
//   // --- DATA DERIVATION ---
//   const { map: GRID_MAP, totalRows, totalCols, pivotCol, keyword } = useMemo(() => {
//     if (!data || data.length === 0) return { map: [], totalRows: 0, totalCols: 0, pivotCol: 0, keyword: '' };

//     // 0. Derive Keyword
//     const keyword = data.map(d => d.answer[d.keyIndex]).join('');
...
//     return { map, totalRows, totalCols, pivotCol, keyword };
//   }, [data]);

//   // State
//   const [userGrid, setUserGrid] = useState(() => 
//     Array(totalRows).fill(null).map(() => Array(totalCols).fill(''))
//   );
  
//   // Re-init state when data/grid changes size
//   useEffect(() => {
//      setUserGrid(Array(totalRows).fill(null).map(() => Array(totalCols).fill('')));
//      setElapsedTime(0);
//      setIsSolved(false);
//      setCheckMode(false);
//      if (data && data.length > 0) {
//         const firstRowItem = data[0];
//         setActiveCell({ row: 0, col: pivotCol - firstRowItem.keyIndex });
//      }
//   }, [data, totalRows, totalCols, pivotCol]);

//   const [activeCell, setActiveCell] = useState({ row: 0, col: -1 }); 
...
//       return data[activeCell.row]?.question || "";
//   }, [activeCell.row, data]);
...
//     if (!/^[A-Z]$/.test(upperChar) && char !== '') return;
...
//           } else {
//              const nextStart = pivotCol - data[nextRow].keyIndex;
...
//           } else {
//              const prevStart = pivotCol - data[prevRow].keyIndex;
...
//       const cellData = GRID_MAP[r][c];
//       if (!cellData) return "invisible border-none";

//       const isActive = activeCell.row === r && activeCell.col === c;
//       const isKeyColumn = c === pivotCol;
...
//             </div>
//             <div>
//               <h1 className="text-xl font-bold tracking-tight">Keyword Puzzle</h1>
//               <div className="flex items-center gap-2 text-xs text-slate-400 mt-0.5">
//                 <span className="bg-slate-800 px-1.5 py-0.5 rounded text-amber-400 font-mono font-bold tracking-wider">{keyword}</span>
//               </div>
//             </div>
//           </div>
...
//                                         {cIndex === (pivotCol - data[rIndex].keyIndex) && (
...
//                   {/* Confetti effect placeholder using gradient */}
