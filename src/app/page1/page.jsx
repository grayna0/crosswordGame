...
let current_crossword_data = CROSSWORD_DATA;
if (current_page === '/2') {
    current_crossword_data = CROSSWORD_DATA_2;
}
...
const calculateGridLayout = () => {
    ...
    const pivotCol = Math.max(...current_crossword_data.map(d => d.keyIndex));
    
    // 2. Tính tổng số cột cần thiết
    // Cần tìm từ dài nhất về phía bên phải cột trụ
    let maxRightExtend = 0;
    current_crossword_data.forEach(item => {
    ...
    const totalRows = current_crossword_data.length;
    ...
    current_crossword_data.forEach((item, rowIndex) => {
...
const { map: GRID_MAP, totalRows, totalCols, pivotCol } = calculateGridLayout();
...
export default function Page1() {
  ...
  const [userGrid, setUserGrid] = useState(() => {
    ...
    current_crossword_data.forEach((item, rIndex) => {
  ...
  useEffect(() => {
    const firstRowItem = current_crossword_data[0];
  ...
  const activeQuestion = useMemo(() => {
      return current_crossword_data[activeCell.row]?.question || "";
  }, [activeCell.row]);

  // Check logic for Prank (Row 5)
  useEffect(() => {
    // Check if first 5 rows (indices 0-4) are correct
    if (totalRows < 5) return;
    
    let allFiveCorrect = true;
    for (let r = 0; r < 5; r++) {
        const item = current_crossword_data[r];
        const startCol = pivotCol - item.keyIndex;
        // Check this row
  ...
  const handleInputChange = (value, rIndex, cIndex) => {
    if (isSolved) return;
    ...
    newGrid[rIndex][cIndex] = upperChar;
    setUserGrid(newGrid);

    // Auto Advance logic
    if (upperChar !== '') {
      moveFocus(rIndex, cIndex, 1);
    }
  };
  ...
  const moveFocus = (r, c, step) => {
    ...
    if (step > 0 && r < totalRows - 1) {
        const nextRow = r + 1;
        const nextItem = CROSSWORD_DATA[nextRow];
    ...
    if (e.key === 'Backspace') {
      ...
      if (row < totalRows - 1) {
          ...
          if (GRID_MAP[nextRow][col]) {
             ...
             const nextStart = pivotCol - CROSSWORD_DATA[nextRow].keyIndex;
      ...
      if (row > 0) {
          ...
          if (GRID_MAP[prevRow][col]) {
             ...
             const prevStart = pivotCol - CROSSWORD_DATA[prevRow].keyIndex;
      ...
      <main className="flex-1 max-w-5xl mx-auto w-full  flex flex-col items-center">
                                ...
                                {cellData && (
                                    <>
                                        ...
                                        <input 
                                            ...
                                            type="text"
                                            className="font700 w-full h-full bg-transparent text-center focus:outline-none caret-transparent cursor-pointer p-0 font-sans"
        ...
        {showPrankButton && (
            <button 
                ...
                className="mt-6 px-8 py-3 bg-gradient-to-r from-green-400 to-emerald-600 text-white font-bold text-lg rounded-full shadow-xl animate-bounce hover:scale-110 transition-transform border-4 border-white ring-4 ring-green-200"
            >
